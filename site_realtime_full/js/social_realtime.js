...
export async function ensureProfile(){
  ...
  const base = { displayName: '匿名用户', bio:'', createdAt: now() };
  if(email){ base.email = email; }
  const snap = await getDoc(uref);
  if(!snap.exists()){ await setDoc(uref, base, { merge:true }); }
  else if(email && !snap.data().email){ await setDoc(uref, { email: email }, { merge:true }); }
